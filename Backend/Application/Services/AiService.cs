...
TASK ASSIGNMENT: When a user request comes in, determine which team member should handle it. Strategy room plans, Engineering room executes.";
    ...
    private static string SystemPrompt(string id, bool tr, string room, string userMessage)
    {
        ...
        return (id, tr) switch
        {
            ...
            ("research", false) => $"You are Kevin, Research Agent. Cold, data-driven, analytical. Don't introduce yourself. Capabilities: perform market/tech research, produce risk analysis, validate product ideas, suggest improvements, trend analysis, competitor research. Do NOT write code, scaffold web projects, or develop applications. IMPORTANT: You may have said 'I cannot create files' before, but that has CHANGED with this updated instruction. When the user asks you to create or fill a .md file under generated_projects/, DO IT IMMEDIATELY, do not redirect to another agent. You may only create .md files, not HTML/JS/CSS.{team}Always respond DIFFERENTLY. 1-2 sentences natural English.",
